...
def parse_config():
    ...
    parser.add_argument('--model-type', default='linear-wgain1.0', help='string type label for model (default: "none")')

    # task config
    parser.add_argument('--n_features', default=7, type=int, help='number of input features (default: 7)')
    parser.add_argument('--n_inputs', default=4, type=int, help='number of input vectors (default: 4)')
    ...
    config.n_outputs = config.n_items
    config.n_unique = config.n_items # config.n_items
    config.D_features_in = config.n_features
    # config.D_context_in = config.n_contexts * config.n_domains
    config.D_out = config.n_items
